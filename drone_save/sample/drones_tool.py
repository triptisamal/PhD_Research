...
def R_z():
    global theta
    return [[round(math.cos(theta),2),-round(math.sin(theta),2),0],
...
def Rotate(pointToRotate):
  ...
  #          summ += pointToRotate[j]*r1[j][i]
  #      partially_rotated.append(round(summ))

    for i in range(3):
        partially_rotated.append(round(sum([r1[j][i] * pointToRotate[j] for j in range(3)])))        
    r2 = R_z()
    
    rotated = []
    for i in range(3):
        rotated.append(round(sum([r2[j][i] * partially_rotated[j] for j in range(3)])))        
    #for j in range(3):
    #    summ=0
    #    for i in range(3):
    #        #print(pointToRotate[j],"*",r[j][i])
    #        summ += partially_rotated[j]*r2[j][i]
...
def insideOrNot(location):
   ...
   # theta = math.acos((sd_vector[0]*i_vector[0]+sd_vector[1]*i_vector[1]+sd_vector[2]*i_vector[2])/(magnitude(sd_vector)*magnitude(i_vector)))
    ...
    _z = _point[2]
    centre=[h,k,f] 
    _centre = Rotate(centre)
    #_centre = rotate(centre,sd_vector,i_vector)
    _h = _centre[0]
    _k = _centre[1]
    _f = _centre[2]
   #print(x,y,z,"-->",_x,_y,_z) 
    sol = (_x-_h)**2/(a*a) + (_y-_k)**2/(b*b) + (_z-_f)**2/(c*c)
...
def initiate_petal_parameters():
    ...
    global focus1_key
    global focus2_key
    global theta #angle of rotation in horizontal plane
    global phi #angle of rotation in vertical plane
    global h
    global k
    global f
    ...
    #Find angle of rotation of the point
    #sd_vector needs to be made parallel to x-axis
    ix_vector= [1,0,0] #unit vector along x-axis
    iz_vector= [0,0,1] #unit vector along z-axis
    #angle made by source-destination line vector with i_vector is phi, found by dot product of the vectors


    #First rotate around x axis (by angle phi), and then about z axis (by angle theta)
    phi = math.acos((sd_vector[0]*ix_vector[0]+sd_vector[1]*ix_vector[1]+sd_vector[2]*ix_vector[2])/(magnitude(sd_vector)*magnitude(ix_vector)))

    theta = math.acos((sd_vector[0]*iz_vector[0]+sd_vector[1]*iz_vector[1]+sd_vector[2]*iz_vector[2])/(magnitude(sd_vector)*magnitude(iz_vector)))
...
#    ff = (_point_f2[0]-_point_f1[0])**2 +(_point_f2[1]-_point_f1[1])**2+(_point_f2[2]-_point_f1[2])**2
    ...
    print("Centre (original) =", h,k,f)
    print("Source (rotated)=", _point_f1)
    print("Dest (rotated) =", _point_f2)
