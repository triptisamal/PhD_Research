...
def update_assertion(claimant,prover,pos,conf,timeofpos):
    
    assertion = {'sender':"DEFAULT", 'agent':"DEFAULT",'position':(0,0,0), 'confidence':0,'time':0}
    assertion['sender'] = claimant
    assertion['agent'] = prover
    assertion['position'] = pos #of prover
    assertion['confidence'] = conf
    assertion['time'] = timeofpos
...
def start_timer(claimant,prover,node_id,duration):
...
#TODO multiple rounds need to be done
    if action == "SEND_PERIODIC_ASSERTION":
       ...
       # print("Agent ",node_id," is creating the packet for position claim at",globalvars.now, "seconds." )
        assertion = update_assertion(node_id,node_id,globalvars.pos[node_id],5,globalvars.now)
  ...
  #      globalvars.event_queue = sorted(globalvars.event_queue, key=lambda x: x['time'])
    ...
    if action == "SEND_CHALLENGE":
       ...
       # print("Agent ",node_id," is challenging e['details']['sender'] and creating the packet for challenge at",globalvars.now, "seconds." )
        ...
        #start timer event
        timer = start_timer(e['details']['claimant'],e['details']['prover'],node_id,challenge['tau'])
    ...
    if action == "TIMER_EXPIRY":
        ...
        if globalvars.max_reached == 1:
            assertion = update_assertion(node_id,e['details']['timer_started_for'],globalvars.pos[e['details']['timer_started_for']],globalvars.confidence_table[node_id][e['details']['timer_started_for']],globalvars.now)
    ...
    for i in range(globalvars.number_of_nodes):
        if i != agent2 and i != agent1:
            if l>0 and m>0 and n>0:
                a = (globalvars.pos[i][0]-globalvars.pos[agent1][0])/l
                b = (globalvars.pos[i][1]-globalvars.pos[agent1][1])/m
                c = (globalvars.pos[i][2]-globalvars.pos[agent1][2])/n
                if a == b and b == c:
                    return 0
                else:
                    continue
            ...
            if l == 0 and m>0 and n>0:
                b = (globalvars.pos[i][1]-globalvars.pos[agent1][1])/m
                c = (globalvars.pos[i][2]-globalvars.pos[agent1][2])/n
                if b == c and globalvars.pos[i][0] == globalvars.pos[agent1][0]:
                    return 0
                else:
                    continue
            ...
            if l > 0 and m == 0 and n>0:
                a = (globalvars.pos[i][0]-globalvars.pos[agent1][0])/l
                c = (globalvars.pos[i][2]-globalvars.pos[agent1][2])/n
                if a == c and globalvars.pos[i][1] == globalvars.pos[agent1][1]:
                    return 0
                else:
                    continue
            ...
            if l > 0 and m > 0 and n == 0:
                a = (globalvars.pos[i][0]-globalvars.pos[agent1][0])/l
                b = (globalvars.pos[i][1]-globalvars.pos[agent1][1])/m
                if b == a and globalvars.pos[i][2] == globalvars.pos[agent1][2]:
                    return 0
                else:
                    continue
            ...
            if l == 0 and m == 0: #z-axis
                if  globalvars.pos[agent2][2] < globalvars.pos[i][2] < globalvars.pos[agent1][2] or globalvars.pos[agent1][2] < globalvars.pos[i][2] < globalvars.pos[agent2][2]:
                    return 0
                else:
                    continue
            if l == 0 and n == 0: #y-axis
                if  globalvars.pos[agent2][1] < globalvars.pos[i][1] < globalvars.pos[agent1][1] or globalvars.pos[agent1][1] < globalvars.pos[i][1] < globalvars.pos[agent2][1]:
                    return 0
                else:
                    continue
            if n == 0 and m == 0: #x-axis
                if  globalvars.pos[agent2][0] < globalvars.pos[i][0] < globalvars.pos[agent1][0] or globalvars.pos[agent1][0] < globalvars.pos[i][0] < globalvars.pos[agent2][0]:
                    return 0
                else:
                    continue
...
def process_event(e):
    ...
    if "ASSERTION" in e['event_id']:
        ...
        for i in range(globalvars.number_of_nodes): #everyone received because wireless communication is infinite
            if e['agent'] != i:
                ret = check_verifiability(e['agent'],i)
                if ret > 0:
                ...
                if ret == 0:
                    ...
                    if globalvars.confidence_table[i][e['details']['agent']] > prev:
                        #add assertion event TODO
                        assertion = update_assertion(i,e['details']['agent'],globalvars.pos[e['details']['agent']],globalvars.confidence_table[i][e['details']['agent']],globalvars.now)

                        event_id = "ASSERTION_%03d" % (globalvars.idn)
                        globalvars.idn += 1

                        e = create_event(event_id,i,assertion,globalvars.now)
                        globalvars.event_queue.append(deepcopy(e))
                  ...
                  #  globalvars.trust_table[i][e['details']['sender']] = 
    ...
    if "CHALLENGE" in e['event_id']:
        ...
        for i in range(globalvars.number_of_nodes):
            if e['details']['prover'] == i:
                node_handler(i,"RESPOND_TO_CHALLENGE",e)
...
def main():
    ...
    filename = "pos_%d.txt" % (globalvars.number_of_nodes)
    ...
    for i in range(globalvars.number_of_nodes):
        node_handler(i,"SEND_PERIODIC_ASSERTION",e);
